...
plugin = lightbulb.Plugin("paranoia", "Get a paranoia question")
...
@lightbulb.command(
    "paranoia", "Get a paranoia question.", aliases=["wyr"], pass_options=True
)
...
async def paranoia_command(
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            f"https://api.truthordarebot.xyz/api/paranoia?rating={rating}"
        ) as resp:
            if resp.status != 200:

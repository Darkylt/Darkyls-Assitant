...
plugin = lightbulb.Plugin("never_have_i_ever", "Play never have I ever")
...
@lightbulb.command(
    "never_have_i_ever", "Play never have I ever.", aliases=["nhie"], pass_options=True
)
@lightbulb.implements(lightbulb.commands.SlashCommand)
async def would_you_rather_command(
    ctx: lightbulb.context.SlashContext, category, language
) -> None:
    if category == "nsfw":
        nsfw = True
        rating = "r"
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            f"https://api.truthordarebot.xyz/api/nhie?rating={rating}"
        ) as resp:
            if resp.status != 200:
